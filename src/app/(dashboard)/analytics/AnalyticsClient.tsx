...
const AnalyticsClient: React.FC = () => {
    ...
    const handleExport = async () => {
        try {
            ...
            const response = await api.get('/dashboard/analytics/export', { params, responseType: 'blob' });
        ...
        <div className="space-y-8 p-4">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                    ...
                    <Button onClick={handleExport} className="bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-bold h-11 px-6 shadow-lg shadow-primary-200">
                        ...
                        {[45, 60, 55, 80, 70, 90, 85, 95, 100, 80, 75, 90].map((h, i) => (
                            <div key={i} className="flex-1 flex flex-col gap-1 items-center group">
                                <div className="w-full bg-slate-100 rounded-t-lg relative overflow-hidden h-full cursor-pointer hover:bg-slate-200 transition-colors">
                                    <div style={{ height: `${h}%` }} className="absolute bottom-0 left-0 right-0 bg-primary-500 rounded-t-lg group-hover:bg-primary-600 transition-all"></div>
                                </div>
